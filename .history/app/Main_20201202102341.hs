...
chartURL :: Int -> Float -> [Float] -> String
chartURL n ser pars =
  let largest = maximum pars
   in -- Google needs values scaled to out-of-100
      let scale = 100 / largest
       in "https://chart.googleapis.com/chart?cht=lc&chs=500x250&chxt=x,x,y,y&chxl=1:|serial%20cutoff|3:|seconds&chco=FF0000,0000FF&chdl=fibSer|fibPar&chxp=1,50|3,50&chxr=0,0,"
            ++ (show $ n + 1)
            ++ "|2,0,"
            ++ (show largest)
            ++ "&chg="
            ++ (show $ 100.0 / (fromIntegral n + 1) * 5.0)
            ++ ",25&chd=t:"
            ++ (show $ scale * ser)
            ++ ","
            ++ (show $ scale * ser)
