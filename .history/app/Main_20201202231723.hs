  ...
  let largest = maximum pars
   in -- Google needs values scaled to out-of-100
      let scale = 100 / largest
       in "https://chart.googleapis.com/chart?cht=lc&chs=500x250&chxt=x,x,y,y&chxl=1:|serial%20cutoff|3:|seconds&chco=FF0000,0000FF&chdl=kSer|kPar&chxp=1,50|3,50&chxr=0,1,"
            ++ (show $ n2 + 1)
            ++ "|2,0,"
            ++ (show largest)
            ++ "&chg="
