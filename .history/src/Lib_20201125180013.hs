...
getNextPowerOf2 x = 2 ^ higherPower x
...
get2ndHalf x halfLength
    | toInteger (numberLength x) <= halfLength = trace (" 2nd half " ++ show x) $ x 
    | otherwise = get2ndHalf (x `mod` 10 ^ (numberLength x - 1)) halfLength
    
get1stHalf :: Integer -> Integer -> Integer
get1stHalf x secondHalf = 
    trace (" 1st half " ++ show ((x - secondHalf) `div` 10 ^ numberLength secondHalf)) $ (x - secondHalf) `div` 10 ^ numberLength secondHalf
...
karatsuba x y = ksba (getNextPowerOf2 (getLargerTwoNumbers x y))
    where ksba n
--            | n <= 1 = trace ("n <= 1 " ++ show x ++ " " ++ show y) $ x * y 
