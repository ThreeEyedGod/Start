...
get2ndHalf x halfLength
    ...
    | otherwise = get2ndHalf ((abs x) `mod` 10 ^ (numberLength x - 1)) halfLength

-- 1st half = left side half    
get1stHalf :: Integer -> Integer -> Integer
get1stHalf x secondHalf = 
   (signum x) *  ((abs x) - secondHalf) `div` 10 ^ numberLength secondHalf
...
--            | otherwise = trace ( "n = " ++ show n ) $ 10 ^ n * u + 10 ^ (n `div` 2) * z + v
            ...
            where
                ...
                diff1 = x1 - x2
                diff2 = y1 - y2
                w = (signum diff1) * (signum diff2) * karatsuba (abs diff1) (abs diff2)
                z = u + v - w
