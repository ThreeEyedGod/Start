...
--  | ab <= 10 = ab * cd
--  | cd <= 10 = ab * cd
    | otherwise = z2 * 10 ^ (digits `div` 2 * 2) + (z0 + z2 - z1) * 10 ^ (digits `div` 2) + z0
 --   let z0 = karatsuba b d
        --z1 = karatsuba (a + b) (c + d)
   --     z1 = karatsuba (a - b) (c - d)
    --    z2 = karatsuba a c
--     in z2 * 10 ^ (digits `div` 2 * 2) + (z1 - z2 - z0) * 10 ^ (digits `div` 2) + z0
    -- in z2 * 10 ^ (digits `div` 2 * 2) + (z0 + z2 - z1) * 10 ^ (digits `div` 2) + z0
  where
    --digits = max (num_digits ab) (num_digits cd)
      digits = getNextPowerOf2 (getLargerTwoNumbers ab cd)
      (a, b) = getHalves ab digits
      --(a, b) = split_digits ab (digits `div` 2)
      --(c, d) = split_digits cd (digits `div` 2)
      (c, d) = getHalves cd digits
      z0 = karatsuba b d
      z1 = karatsuba (a - b) (c - d)
      z2 = karatsuba a c
