...
karatsubaPar ab cd cutoff
  | ab <= 10 = ab * cd
  | cd <= 10 = ab * cd
  | (cutoff >= ab) || (cutoff >= cd) = z0 `pseq` z2 `pseq` z1 `pseq` put_together
  | otherwise = (z0 `par` z2) `pseq` z1 `pseq` put_together
  where
    digits = max (num_digits ab) (num_digits cd)
    (a, b) = split_digits ab (digits `div` 2)
    (c, d) = split_digits cd (digits `div` 2)
...
timeAll n1 n2 = do
  times <- sequence $ map (timeCutoff n1 n2) [(max n1 n2) `div` 1, (max n1 n2) `div` 2,(max n1 n2) `div` 4, (max n1 n2) `div` 16, (max n1 n2) `div` 32]
