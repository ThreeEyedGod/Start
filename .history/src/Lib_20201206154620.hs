...
simpletime :: Integer -> Integer -> Integer -> IO [Float]
simpletime n1 n2 times = do
  timesPar <- sequence $ map (timePar n1 n2) $ map (* n2) [0..times]
  timesSer <- sequence $ map (timeSer n1 n2) [0..times]
  return $ map (fromRational . toRational) timesPar
