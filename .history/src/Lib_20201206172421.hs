...
timePar n1 n2 = do
  ...
  st <- getCurrentTime
  let r = karatsubaPar n1 n2 (fromInteger cpus)
  en <- st `CP.pseq` r `CP.pseq` getCurrentTime
...
mergeMinus :: IO [Float] -> IO [Float] -> IO [Float]
mergeMinus xs     []     = xs
mergeMinus []     ys     = ys
mergeMinus (x:xs) (y:ys) = (fromRational . toRational x - fromRational . toRational y) : mergeMinus xs ys
...
comp n1 n2 times = do
  ...
  return $ map (fromRational . toRational) $ mergeMinus timesPar timesSer
